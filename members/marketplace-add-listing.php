<?php
	...
	if ($_SESSION['marketplace_unlimited'] != 'Y') {
		...
		if ($count >= 3) {
			$error = "You have reached your maximum number of listings. <a href='upgrade.php'>Upgrade</a> or delete existing listings.";
			echo json_encode(array('status' => 'failed', 'error' => $error));
			exit;
		}
	}
	$query = "INSERT INTO marketplace_domains SET user_id='$id', domain='$domain', type='$type', category='$category', link='$link', price='$price', listingdate=CURDATE()";
	$result = mysql_query($query);
